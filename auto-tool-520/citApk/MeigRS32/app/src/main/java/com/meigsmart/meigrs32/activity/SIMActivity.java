...
public class SIMActivity extends BaseActivity implements View.OnClickListener, PromptDialog.OnPromptDialogCallBack {
    ...
    private void showDevice() {
     ...
     // add by maohaojie on 2019.02.18 for bug 23962 end
        ...
        boolean sim2_pass = phoneCount == 2 && resultList1.get(0).equals(fine) && resultList0.get(0).equals(fine);
        boolean sim1_pass = (phoneCount == 1 && resultList0.get(0).equals(fine));
        String imei_show = DataUtil.initConfig(Const.CIT_COMMON_CONFIG_PATH,imeishownumber );
        if (imei_show.equals("0")){
           if(resultList0.get(0).equals(fine)){
               sim1_pass = true;
           }
        }
        if ( sim2_pass|| sim1_pass) {
            isInsert = true;
        } else {
            isInsert = false;
        }
    ...
    private List<String> getResultList(int simId) {
        ...
        /*jicong.wang modify for bug 15896 start {@*/
        if (Build.VERSION.SDK_INT == 30){
            String simCountry = SystemProperties.get("gsm.sim.operator.iso-country");
            LogUtil.d("get sim country for prop,simCountry is "+simCountry);
            if (!TextUtils.isEmpty(simCountry)){
                String[] sims = simCountry.split(",");
                LogUtil.d("sims length = "+sims.length);
                if (simId <= sims.length - 1 && !TextUtils.isEmpty(sims[simId])){
                    resultList.add(sims[simId]);
                } else {
        ...
        /*jicong.wang modify for bug 15896 end @}*/
