...
public final class BatteryService extends SystemService {
    ...
    // < !-- modify for task 23083, Show icon/ notfication when backup battery is low start
    private static final String SUB_BATTERY_VOLT = "sys/class/power_supply/battery/sub_voltage_now";
    private int mLastSubBatteryVolt;
    //modify for task 23083, Show icon/ notfication when backup battery is low end-->
    ...
    private void processValuesLocked(boolean force) {
        ...
        if (force || (mHealthInfo.batteryStatus != mLastBatteryStatus ||
            ...
            //<!--modify for task 23083, Show icon/ notfication when backup battery is low start
            if (shouldSubBatteryLowLocked()) {
                final Intent statusIntent = new Intent("MEIG.subbattery.low");
                statusIntent.setFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
                Log.i(TAG,"sendBroadcast MEIG.subbattery.low");
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        mContext.sendBroadcastAsUser(statusIntent, UserHandle.ALL);
            ...
            if (hideSubBatteryLowLocked()) {
                final Intent statusIntent = new Intent("MEIG.subbattery.low.dismiss");
                statusIntent.setFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
                Log.i(TAG,"sendBroadcast MEIG.subbattery.low.dismiss");
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        mContext.sendBroadcastAsUser(statusIntent, UserHandle.ALL);
    ...
    //<!--modify for task 23083, Show icon/ notfication when backup battery is low start
    private boolean shouldSubBatteryLowLocked() {
        final boolean plugged = mPlugType != BATTERY_PLUGGED_NONE;
        final boolean oldPlugged = mLastPlugType != BATTERY_PLUGGED_NONE;

        String batteryInfo = getBatteryInfo(SUB_BATTERY_VOLT);
        int subBatteryVolt;
        if (batteryInfo.isEmpty()){
            subBatteryVolt = -9999;
            Slog.d(TAG,"batteryInfo is empty!!! ");
        } else {
            subBatteryVolt = Integer.parseInt(batteryInfo);
        }
        Slog.d(TAG,"subBatteryVolt: " + subBatteryVolt);

        if(!plugged && mHealthInfo.batteryStatus != BatteryManager.BATTERY_STATUS_UNKNOWN
                && subBatteryVolt != -9999 && subBatteryVolt < 3900
                && (oldPlugged || mLastSubBatteryVolt >= 3900)){
            return true;
        }
        mLastSubBatteryVolt = subBatteryVolt;
        return false;
    }

    private boolean hideSubBatteryLowLocked(){
        final boolean plugged = mPlugType != BATTERY_PLUGGED_NONE;
        final boolean oldPlugged = mLastPlugType != BATTERY_PLUGGED_NONE;
    ...
    private String getBatteryInfo(String path) {
        ...
        try {
            ...
            batteryInfo = new String(buffer, 0, len);
            file.close();
            batteryInfo = batteryInfo.trim();
        } catch (Exception e) {
            e.printStackTrace();
            Slog.d(TAG, "Error write: " + e.toString());
        }
        return batteryInfo;
    }
    //modify for task 23083, Show icon/ notfication when backup battery is low end-->
