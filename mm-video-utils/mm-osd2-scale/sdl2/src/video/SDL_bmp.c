...
SDL_SaveBMP_RW(SDL_Surface * saveme, SDL_RWops * dst, int freedst)
{
    ...
    if (dst) {
        ...
        //LOGE("========> save32bit trueBitsPerPixel:%d , Amask:0x%x, flags:0x%x<======\n", saveme->format->BitsPerPixel, saveme->format->Amask, saveme->map->info.flags);
    ...
    if (save32bit) {
        //LOGE("========>32-bit BMP <======\n");
        saveLegacyBMP = SDL_GetHintBoolean(SDL_HINT_BMP_SAVE_LEGACY_FORMAT, SDL_TRUE);
    }
