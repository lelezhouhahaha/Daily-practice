...
int RGB2YUV(unsigned long w, unsigned long h, unsigned char *rgbData, unsigned char * y, unsigned char * u, unsigned char *v)
{
	...
	if (utemp == NULL || vtemp == NULL) {
		LOGE("[xuhao] RGB2YUV malloc buffer failed \n");
		return -1;
	}
	...
	unsigned long i,nr=0,ng=0,nb=0,nSize;
	//对每个像素进行 rgb -> yuv的转换
	for (i=0,nSize=0;nSize<w*h*3;nSize+=3)
	{
		...
		ng = rgbData[nSize+1];
		nr = rgbData[nSize+2];
		if(nr > 256 || ng > 256 || nb > 256){
			LOGE("RGB2YUV nr=%ld ng=%ld nb=%ld",nr,ng,nb);
			if(nr > 256)
				nr = 256;
			if(ng > 256)
				ng = 256;
	...
	if(rgbData) {
		free(rgbData);//Memory allocated in OSD_ReadBmpFile is not released
		rgbData = NULL;
	}
	return 0;
}
...
bool ReadBmpFromBuf(char *bmpBuf)
{
	...
	if (((bmp.info_header.biWidth*bmp.info_header.biBitCount/8)%4) == 0)
