...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
int vt_frame_read(void *frame_buffer, void *buffer, int width, int height, int config,
        unsigned int format) {

    int bytes = width;

    int result = 0, color_format = 0;
	char *f_buffer = (char *)frame_buffer;
    switch (format) {
    case QOMX_COLOR_FORMATYUV420PackedSemiPlanar32m:
        color_format = COLOR_FMT_NV12;
        break;
        ...
        if (bytes > 0) {
            if (!config) {
                if (color_format == COLOR_FMT_NV12) {
					...
					VTEST_MSG_ERROR("don't support color format");
					result = -1;
				}
			} else {
				VTEST_MSG_ERROR("don't run to here!!!");
			}
        } else {
            VTEST_MSG_ERROR("Bytes must be > 0");
            result = -1;
...
int vt_nv21_to_nv12(void *frame_buffer, int width, int height) {

    int bytes = width;
    int result = 0;
	int frame_size = width * height;
	char *f_buffer = (char *)frame_buffer;
	char *p_uv =  f_buffer + frame_size;
	char U, V;
