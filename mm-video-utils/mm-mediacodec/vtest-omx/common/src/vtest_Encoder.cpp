...
OMX_ERRORTYPE Encoder::Configure(CodecConfigType *pConfig,
    ...
    m_nPFrame = pConfig->nPFrames;
    m_nFramerate = pConfig->nFramerate;
    m_pVideoCore = (VideoCore*)pConfig->pVideoCore;
	...
	OMX_INIT_STRUCT(&idr_period, OMX_VIDEO_CONFIG_AVCINTRAPERIOD);
        ...
        if (result == OMX_ErrorNone && pConfig->sHybridHP.nHpLayers) {
            result = SetHybridHp(pConfig);
        ...
        if (result == OMX_ErrorNone && pConfig->sHybridHP.nHpLayers) {
            result = SetHybridHp(pConfig);
            FAILED1(result, "Failed : Setting Hybrid HP for HEVC")
        }
    }
            ...
            if (result == OMX_ErrorNone) {
                ...
                if (e.bEnabled == OMX_FALSE) {
                    ...
                    VTEST_MSG_HIGH("SetParameter OMX_QcomIndexParamIndexExtraDataType (%x) = %d",
                            e.nIndex, e.bEnabled);
                    OMX_INIT_STRUCT_SIZE(&e, QOMX_INDEXEXTRADATATYPE);
                    result = OMX_SetParameter(m_hEncoder,
                            (OMX_INDEXTYPE)OMX_QcomIndexParamIndexExtraDataType,
                            (OMX_PTR)&e);
                    if (result != OMX_ErrorNone)
...
OMX_ERRORTYPE Encoder::SetBuffer(BufferInfo *pBuffer, ISource *pSource) {
    ...
    if (pSource == m_pSource) { //input port
        ...
        VTEST_MSG_LOW("%s ==> %s: FTB size=%u, (%p %p)", pSource->Name(), Name(),
                (unsigned int)pBuffer->pHeaderOut->nFilledLen, pBuffer->pHeaderIn, pBuffer->pHeaderOut);
...
OMX_ERRORTYPE Encoder::ThreadRun(OMX_PTR pThreadData) {
    ...
    while (!m_bThreadStop) {
		VTEST_MSG_MEDIUM("thread Sleep 100");
        Sleeper::Sleep(100);
    }
...
OMX_ERRORTYPE Encoder::SetState(
	...
	VTEST_MSG_MEDIUM("setState m_eState = %s, eState = %s, m_eStatePending = %s...", OMX_STATE_NAME(m_eState), OMX_STATE_NAME(eState), OMX_STATE_NAME(m_eStatePending));
    // check for pending state transition
    if (m_eState != m_eStatePending) {
		VTEST_MSG_MEDIUM("wait for state %s...", OMX_STATE_NAME(m_eStatePending));
...
/*
    ...
    if(m_nBitrate != m_pVideoCore->nBitrate) {
        VTEST_MSG_HIGH("change bitrate from %d to %d.", m_nBitrate, m_pVideoCore->nBitrate);
        m_nBitrate = m_pVideoCore->nBitrate;
        DynamicProperties.frame_num                          = m_nInBufferCount;
        DynamicProperties.config_param                       = OMX_IndexConfigVideoBitrate;
        DynamicProperties.config_data.bitrate.nPortIndex     = (OMX_S32)PORT_INDEX_OUT;
        DynamicProperties.config_data.bitrate.nEncodeBitrate = m_nBitrate;
        result = SetDynamicConfig(DynamicProperties.config_param, &DynamicProperties.config_data);

        if (result == OMX_ErrorNone) {
            VTEST_MSG_MEDIUM(
                "Set Dynamic Config Successfully. Frame count: %d. Config Index: %x",
                DynamicProperties.frame_num,
                DynamicProperties.config_param);
        } else {
            VTEST_MSG_ERROR(
    ...
    if(m_nPFrame != m_pVideoCore->nPFrames) {
        VTEST_MSG_HIGH("change P frame from %d to %d.", m_nPFrame, m_pVideoCore->nPFrames);
        m_nPFrame = m_pVideoCore->nPFrames;
        DynamicProperties.frame_num                          = m_nInBufferCount;
        DynamicProperties.config_param                       = (OMX_INDEXTYPE)QOMX_IndexConfigVideoIntraperiod;
        DynamicProperties.config_data.intraperiod.nPortIndex = (OMX_S32)PORT_INDEX_OUT;
        DynamicProperties.config_data.intraperiod.nPFrames   = m_nPFrame;
        DynamicProperties.config_data.intraperiod.nBFrames   = 0;
        DynamicProperties.config_data.intraperiod.nIDRPeriod = 1;
    ...
    if(m_nFramerate != m_pVideoCore->nFrameRate) {
        VTEST_MSG_HIGH("change framerate from %d to %d.", m_nFramerate, m_pVideoCore->nFrameRate);
        m_nFramerate = m_pVideoCore->nFrameRate;
        result = ChangeFrameRate(m_nFramerate);
        if (result) {
...
OMX_ERRORTYPE Encoder::SetDynamicConfig(OMX_INDEXTYPE config_param, DynamicConfigData* config_data) {
    ...
    switch(config_param_qcom_ext) {
        ...
        case QOMX_IndexConfigVideoIntraperiod:
            OMX_INIT_STRUCT_SIZE(&config_data->intraperiod, QOMX_VIDEO_INTRAPERIODTYPE);
            result = OMX_SetConfig(m_hEncoder, config_param, (OMX_PTR) &config_data->intraperiod);
            break;
        default:
            break;
    }
...
OMX_ERRORTYPE Encoder::SetHierPNumLayers(OMX_U32 nHierPNumLayers) {
    ...
    if (m_sConfig.preferredOmxIndex.IndexParamAndroidVideoTemporalLayers) {
        ...
        OMX_VIDEO_PARAM_ANDROID_TEMPORALLAYERTYPE layerParams;
        OMX_U32 cumulativeBitrate = 0;
        OMX_INIT_STRUCT(&layerParams, OMX_VIDEO_PARAM_ANDROID_TEMPORALLAYERTYPE);
        layerParams.nSize = sizeof(OMX_VIDEO_PARAM_ANDROID_TEMPORALLAYERTYPE);
        layerParams.nPortIndex = PORT_INDEX_OUT;
        result = OMX_GetParameter(m_hEncoder,
                (OMX_INDEXTYPE)OMX_IndexParamAndroidVideoTemporalLayers,
                &layerParams);
        if (result != OMX_ErrorNone) {
            FAILED1(result, "Error: Get OMX_VIDEO_PARAM_ANDROID_TEMPORALLAYERTYPE");
            return result;
        }

        layerParams.nTemporalPLayerCountActual = nHierPNumLayers;
        layerParams.nTemporalBLayerCountActual = 0;
        layerParams.bTemporalLayerBitrateRatioSpecified =
            m_sConfig.nRcDistribution[0] != 0 ? OMX_TRUE : OMX_FALSE;

        if (layerParams.bTemporalLayerBitrateRatioSpecified == OMX_TRUE) {
            for (unsigned int i = 0; i < m_sConfig.nHierPNumLayers; i++) {
                layerParams.nTemporalLayerBitrateRatio[i] = m_sConfig.nRcDistribution[i] + cumulativeBitrate;
                cumulativeBitrate = layerParams.nTemporalLayerBitrateRatio[i];
                VTEST_MSG_LOW("Layer Bit-rate setting to Component : Layer =  %d Bit Rate = %d",
                        i, layerParams.nTemporalLayerBitrateRatio[i]);
            }
        }
        OMX_INIT_STRUCT_SIZE(&layerParams, OMX_VIDEO_PARAM_ANDROID_TEMPORALLAYERTYPE);
        result = OMX_SetParameter(m_hEncoder,
                (OMX_INDEXTYPE)OMX_IndexParamAndroidVideoTemporalLayers,
                &layerParams);
        FAILED1(result, "Error: Set OMX_IndexParamAndroidVideoTemporalLayers");
        return result;
    } else if (nHierPNumLayers) {
...
OMX_ERRORTYPE Encoder::EmptyDoneCallback(OMX_IN OMX_HANDLETYPE hComponent,
    ...
    VTEST_MSG_MEDIUM("EBD size=%u, pHeader->nFlags = 0x%x", (unsigned int)pHeader->nFilledLen, pHeader->nFlags);
...
OMX_ERRORTYPE Encoder::FillDoneCallback(OMX_IN OMX_HANDLETYPE hComponent,
    ...
    VTEST_MSG_MEDIUM("FBD size=%u, pHeader->nFlags = 0x%x", (unsigned int)pHeader->nFilledLen, pHeader->nFlags);
    result = pThis->m_pBufferManager->GetBuffer(
...
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////
OMX_ERRORTYPE Encoder::SetHybridHp(CodecConfigType *pConfig) {
