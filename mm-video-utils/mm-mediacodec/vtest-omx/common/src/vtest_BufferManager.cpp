...
BufferManager::~BufferManager() {
    ...
    VTEST_MSG_HIGH("~BufferManager start");
    ...
    VTEST_MSG_HIGH("~BufferManager done");
}
...
OMX_ERRORTYPE BufferManager::SetupBufferPool(ISource *pNodeSrc, ISource *pNodeSink) {
	...
	VTEST_MSG_LOW("sPortCapSrc.nMinBufferCount : %d, sPortCapSink.nMinBufferCount : %d, sPortCapSrc.nExtraBufferCount : %d, sPortCapSink.nExtraBufferCount : %d",
		sPortCapSrc.nMinBufferCount, sPortCapSink.nMinBufferCount, sPortCapSrc.nExtraBufferCount, sPortCapSink.nExtraBufferCount);
...
BufferPool* BufferManager::GetBufferPool(ISource *pNode, OMX_U32 ePortIndex) {
    ...
    BufferPool *pBufferPool = NULL;
    for (OMX_U32 i = 0; i < m_pBufferPools.size(); i++) {
		VTEST_MSG_LOW("BufferPool[%d] for %s on %s",
                    i, pNode->Name(), OMX_PORT_NAME(ePortIndex));
        pBufferPool = m_pBufferPools[i];
        Mutex::Autolock autoLock(pBufferPool->pLock);
