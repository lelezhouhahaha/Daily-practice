...
OMX_ERRORTYPE EncoderFileSink::Configure(CodecConfigType *pConfig,
    ...
    //m_notify_num = pConfig->notify_num;
    m_pVideoCore = (VideoCore*)pConfig->pVideoCore;
    if(m_pVideoCore) {
        m_bUpload = m_pVideoCore->bUpload;
    }
...
#endif
    property_get("persist.meigcam.uploadfilesave", value, "0");
    if (!atoi(value) && m_bUpload) {
        VTEST_MSG_HIGH("Disable upload stream local save at default.");
        m_bPrvFileSave = OMX_FALSE;
    } else {
        m_bPrvFileSave = OMX_TRUE;
    }
...
#if 0
    if (!m_pVideoCore->bPreFile && m_bPrvFileSave && pFileName != NULL &&
...
OMX_ERRORTYPE EncoderFileSink::ThreadRun(OMX_PTR pThreadData) {
    ...
    for (OMX_S32 i = 1; !m_bThreadStop; i++) {
        ...
        if(m_pVideoCore->eState == State_Stopped && i == 1 && m_pVideoCore->bPreFile) {
            memcpy(m_pVideoCore->sFileHead, pHeader->pBuffer + pHeader->nOffset, pHeader->nFilledLen);
            m_pVideoCore->sHeadSize = pHeader->nFilledLen;
            VTEST_MSG_HIGH("File head size %d", m_pVideoCore->sHeadSize);
        } else {
            writeFrame(pHeader);
        }
        VTEST_MSG_LOW("wrote frame %u, pHeader->nFlags = 0x%x", (unsigned int)i, pHeader->nFlags);
    ...
    if(m_pVideoCore->eState != State_Stopped && m_pVideoCore->bPreFile) {
        m_pVideoCore->bPreFile = OMX_FALSE;
        m_pVideoCore->nPreLength = m_pVideoCore->pLinkQueue->GetSize();
        m_pVideoCore->Write();
    }
    //clean up
...
/*
    if ((!m_pVideoCore->bPreFile && m_bPrvFileSave == OMX_TRUE && m_pFile == NULL) || pHeader->nFilledLen <= 0) {
        VTEST_MSG_ERROR("received bytes %u...", (unsigned int)pHeader->nFilledLen);
...
*/
    ...
    if (m_bSecureSession) {
        ...
        if(m_bPrvFileSave) {
            if(m_pVideoCore->bPreFile) {
                node.data = pHeader->pBuffer + pHeader->nOffset;
                node.data_size = pHeader->nFilledLen;
                if(m_pVideoCore->pTimeQueue != NULL && !m_pVideoCore->pTimeQueue->empty()) {
                    node.time = m_pVideoCore->pTimeQueue->front();
                    m_pVideoCore->pTimeQueue->pop();
                }
                node.next = NULL;
                result = m_pVideoCore->pLinkQueue->Enqueue(&node);
                nBytes = pHeader->nFilledLen;
            } else {

                memcpy(m_pVideoCore->video_buf + m_pVideoCore->memcpy_offset, pHeader->pBuffer + pHeader->nOffset, pHeader->nFilledLen);

                m_pVideoCore->memcpy_offset = m_pVideoCore->memcpy_offset+pHeader->nFilledLen;
/*
...
*/
		...
		if(m_bUpload)
		{
            VTEST_MSG_LOW("upload %d bytes.", nBytes);
            m_pVideoCore->pfnUploadCB((char *)pHeader->pBuffer, nBytes);
		}
    }
/*
    if (result != OMX_ErrorNone) {
...
*/
#if 0
	VTEST_MSG_ERROR("zhazhao pHeader->pBuffer %p pHeader->nFlags %d QCOM_FLAG_FREE_BUFFER %d nFlags & FREE_BUFFER %d",
	              pHeader->pBuffer,
	              pHeader->nFlags,
	              QCOM_FLAG_FREE_BUFFER,
	              pHeader->nFlags & QCOM_FLAG_FREE_BUFFER);
#endif
